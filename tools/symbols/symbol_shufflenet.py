...
def shufflenet_unit(data, num_filter, stride, dim_match, name, num_group, bn_mom=0.9, workspace=512, memonger=False):
    ...
    # Depthwise Conv, method 2
    sc1_channels = mx.sym.split(data=sc1, axis=1, num_outputs=int(num_filter*0.25))
    dw_outs = [mx.sym.Convolution(data=sc1_channels[i], num_filter=1, pad=(1, 1), kernel=(3, 3),
             no_bias=True, workspace=workspace, name=name+'_depthwise_kernel'+str(i)) for i in range(int(num_filter*0.25))]
