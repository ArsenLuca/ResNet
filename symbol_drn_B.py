...
def drn_b(units, num_stage, filter_list, num_class, data_type, bottle_neck=True, bn_mom=0.9, workspace=512, memonger=False):
    ...
    # end stages appended, using resnet unit
    for e in range(num_stage, num_stage+2):
        body = drn_unit(body, filter_list[-1], (2 if e==num_stage else 1, 2 if e==num_stage else 1), 
                            (1, 1), False, name='stage%d_unit%d' % (e + 1, 1), 
                            bottle_neck=bottle_neck, workspace=workspace, memonger=memonger)
